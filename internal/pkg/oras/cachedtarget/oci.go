...
import (
	...
	"github.com/unbasical/doras-server/internal/pkg/oras/graph"
	"github.com/unbasical/doras-server/internal/pkg/oras/manifestutil"
	"github.com/unbasical/doras-server/internal/pkg/oras/resolver"
	"io"
	"maps"
	"os"
	"path"
...
func (s *CachedStore) Fetch(ctx context.Context, target ocispec.Descriptor) (io.ReadCloser, error) {
	s.sync.RLock()
...
func (s *CachedStore) Push(ctx context.Context, expected ocispec.Descriptor, reader io.Reader) error {
	s.sync.RLock()
	defer s.sync.RUnlock()
...
// being deleted.
func (s *CachedStore) Delete(ctx context.Context, target ocispec.Descriptor) error {
	s.sync.Lock()
	defer s.sync.Unlock()
...
func (s *CachedStore) delete(ctx context.Context, target ocispec.Descriptor) ([]ocispec.Descriptor, error) {
...
func (s *CachedStore) Tag(ctx context.Context, desc ocispec.Descriptor, reference string) error {
	s.sync.RLock()
	defer s.sync.RUnlock()
...
func (s *CachedStore) Resolve(ctx context.Context, reference string) (ocispec.Descriptor, error) {
	s.sync.RLock()
	defer s.sync.RUnlock()
...
func (s *CachedStore) Untag(ctx context.Context, reference string) error {
	if reference == "" {
...
// store.
func (s *CachedStore) Predecessors(ctx context.Context, node ocispec.Descriptor) ([]ocispec.Descriptor, error) {
	s.sync.RLock()
	defer s.sync.RUnlock()
...
func (s *CachedStore) Tags(ctx context.Context, last string, fn func(tags []string) error) error {
	s.sync.RLock()
	defer s.sync.RUnlock()
...
// ensureOCILayoutFile ensures the `cachedtarget-layout` file.
func (s *CachedStore) ensureOCILayoutFile() error {
...
func (s *CachedStore) loadIndexFile(ctx context.Context) error {
...
func (s *CachedStore) SaveIndex() error {
	s.sync.RLock()
	defer s.sync.RUnlock()
...
func (s *CachedStore) writeIndexFile() error {
...
func (s *CachedStore) GC(ctx context.Context) error {
	s.sync.Lock()
	defer s.sync.Unlock()
...
func (s *CachedStore) gcIndex(ctx context.Context) error {
	tagResolver := resolver.NewMemory()
	graph := graph.NewMemory()
	tagged := set.New[digest.Digest]()
...
func (s *CachedStore) isTagged(desc ocispec.Descriptor) bool {

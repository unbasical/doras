...
import (
	...
	"errors"
	"fmt"
	"github.com/unbasical/doras-server/pkg/oras/cas"
...
func TestStoreInterface(t *testing.T) {
	var store interface{} = &CachedStore{}
...
func TestStore_Success(t *testing.T) {
	...
	tempDir := t.TempDir()
	tempDirFsRoot := t.TempDir()
	s, err := New(tempDir, tempDirFsRoot, false)
	if err != nil {
...
func TestStore_NotExistingRoot(t *testing.T) {
	...
	fsRoot := filepath.Join(tempDir, "fs-root")
	_, err := New(root, fsRoot, false)
	if err != nil {
		t.Fatal("New() error =", err)
	}
...
func TestStore_ContentNotFound(t *testing.T) {
	...
	tempDir := t.TempDir()
	s, err := New(tempDir, t.TempDir(), false)
	if err != nil {
		t.Fatal("New() error =", err)
	}
	ctx := context.Background()
...
func TestStore_BadIndex(t *testing.T) {
	...
	_, err := New(tempDir, t.TempDir(), false)
	if err == nil {
...
func TestStore_ExistingStore(t *testing.T) {
	...
	anotherS, err := New(tempDir, t.TempDir(), false)
	if err != nil {
		t.Fatal("New() error =", err)
	}
...
func TestCopy_MemoryToOCI_FullCopy(t *testing.T) {
	...
	tempDir := t.TempDir()
	dst, err := New(tempDir, t.TempDir(), false)
	if err != nil {
...
func TestCopyGraph_OCIToMemory_FullCopy(t *testing.T) {
	tempDir := t.TempDir()
	src, err := New(tempDir, t.TempDir(), false)
	if err != nil {
		t.Fatal("OCI.New() error =", err)
	}

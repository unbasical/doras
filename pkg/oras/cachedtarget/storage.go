...
func (s *Storage) ingest(expected ocispec.Descriptor, content io.Reader) (path string, ingestErr error) {
	...
	if err := ioutil.CopyBuffer(writer, content, *buf, expected); err != nil {
		fmt.Printf("fetching failed: artifact=%s new-bytes=%d cached-bytes=%d  artifact-size=%d in total\n", expected.Digest, writer.BytesWritten(), writer.GetSkipBytes(), expected.Size)
		return "", fmt.Errorf("failed to ingest: %w", err)
	}
	fmt.Printf("fetching succeeded: artifact=%s new-bytes=%d cached-bytes=%d  artifact-size=%d in total\n", expected.Digest, writer.BytesWritten(), writer.GetSkipBytes(), expected.Size)
